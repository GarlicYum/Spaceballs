...
World::World()
	...
	bigEnemyM(bigEnemySurface, bigEnemyBulletM, bigEnemyExplo, bigEnemyExploSound, bigBulletAnim, bigEnemyFlash),
	...
	boss(bossAnim, bossLeftBulletM, bossRightBulletM, bossCenterBulletM, bigBulletAnim, lightBallAnim,
		bossExplo, bossPreExplo, shipExplodeSound, bossFlash),
	...
	smallEnemyM(smallEnemyExhaust, smallEnemyExplode, smallExplo, smallLeftBulletM, smallRightBulletM, smallBulletAnim, smallEnemyFlash),
...
void World::CheckCollisions(float dt)
{
	...
	case PlayState:
		...
		for (int i = 0; i < bigEnemyM.GetBigCount(); ++i)
		{
			auto& bigEnemy = bigEnemyM.GetBigShip(i);
			if (!bigEnemy.IsAlive())
				continue;
			const auto bigShipRect = bigEnemy.GetCollisionRect();
			if (bigEnemy.IsAlive())
			{
				if (IsColliding(shipRect, bigShipRect) && ship.IsAlive() && !bigEnemy.GetCoolDown())
				{
					shipCollideSound.Play(1.1f, 0.9f);
